...
void USART_ClearITPendingBit(USART_TypeDef *usart,unsigned int sr)
{
...
static rt_err_t rt_serial_init (rt_device_t dev)
{
    ...
    if (!(dev->flag & RT_DEVICE_FLAG_ACTIVATED))
    {
        ...
        /* Enable USART */
				uart->uart_device->CR1 |=  USART_CR1_UE;
...
void rt_hw_serial_isr(rt_device_t device)
{
    ...
    if((uart->uart_device->SR&USART_IT_RXNE) != RESET)
    {
    ...
    if (uart->uart_device->SR &= USART_IT_TC != RESET)
    {
        /* clear interrupt */
...
void rt_hw_serial_dma_tx_isr(rt_device_t device)
{
    ...
    else
    {
        ...
        //DMA_Cmd(uart->dma_tx->dma_channel, DISABLE);
			uart->dma_tx->dma_channel->CR&=~DMA_SxCR_EN;
    }
}
